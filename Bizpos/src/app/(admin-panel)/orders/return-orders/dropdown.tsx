...
import { Orders, Settings } from "@/types/shared";
...
import { TotalsOfOrder } from "../orders-list/dropdown";
import PrintDeliverySlip from "@/components/print-pages/delivery-slip-print";
import { formatDate } from "date-fns";
import { returnOrderUndo } from "./action";
...
export const OrdersReturnDropdown: React.FC<Prop> = ({ order }) => {
  ...
  const handleUndoReturn = async () => {
    if (await confirmation("Are you sure you want to undo sales return?")) {
      const data = await ensureOrderData();
      if (data && data.length > 0) {
        await returnOrderUndo(data, order.order_id);
      } else {
        toast({
          title: "Order data unavailable",
          description: "Unable to load order details for undo return.",
          variant: "destructive",
        });
      }
    }
  ...
  // console.log(orderData, "from return order dropdown");
  return (
    <div>
      <DropdownMenu open={menuOpen} onOpenChange={setMenuOpen}>
        ...
        <DropdownMenuContent>
          ...
          >
            ...
            <span className="ml-2 text-red-600">Undo Return</span>
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
